...
interface SalaryShareFormValues {
  position: string;
  sector: string;
  city: string;
...
export interface SalaryShareFormInitialValues {
  position?: string;
  sector?: string;
  city?: string;
  yearsOfExperience?: number;
...
interface SalaryShareFormProps {
  initialValues?: SalaryShareFormInitialValues;
  title?: string;
  description?: string;
  onSubmitted?: () => void;
}

export function SalaryShareForm({
  initialValues,
  title = 'Maaş Bilgileri',
  description = 'Tüm alanlar zorunludur',
  onSubmitted,
}: SalaryShareFormProps) {
  ...
  async function onSubmit(data: SalaryShareFormValues) {
    setIsLoading(true);
    try {
